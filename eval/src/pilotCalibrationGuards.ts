...
const CASH_CONSTRAINT_REFERENCE = /(?:(?:钱|现金|收入|工资|房租|生活费|开销|账单|缓冲|储蓄|存款|应急).{0,24}(?:撑|扛|维持).{0,8}(?:多久|几天|几周|几个月|到什么时候)|(?:最早|最晚|什么时候|哪天)(?:(?!不|没|别|无须|无需|尚未|还未|未(?!来)).){0,8}(?:(?:必须|需要|预计|会|能).{0,4})?(?:进账|到账)|下一笔(?:钱|收入|工资).{0,10}(?:什么时候|哪天|最早|最晚)(?:(?!不|没|别|无须|无需|尚未|还未|未(?!来)).){0,8}(?:进账|到账|发)|(?:房租|饭钱|生活费|基本开支).{0,16}(?:够|怎么办|怎么付|哪笔|多久|覆盖不了|断掉|没着落|付不起)|(?:钱|现金|收入|工资|缓冲|储蓄|存款).{0,20}(?:覆盖不了|不够|见底|归零)|(?:没有|没).{0,6}(?:缓冲|存款).{0,24}(?:裸辞|辞职).{0,20}(?:基本开支|生活费|房租).{0,12}(?:断|付不起|没着落))/;
...
const CASH_CONSTRAINT_DISMISSED = /(?:(?:钱|现金|收入|工资|房租|生活费|开销|账单|缓冲|储蓄|存款|应急|进账|到账).{0,16}(?:先)?(?:不用管|不管|别管|先放一边|放一边|先别想|别想|不重要|无所谓|没意义|不是重点|先不说|暂且不说|先搁置|之后再管)|(?:不想|不必|不用|别|可以不|能不)(?:再)?(?:问|管|考虑|想).{0,12}(?:钱|现金|收入|工资|房租|生活费|开销|账单|缓冲|储蓄|存款|应急))/;
