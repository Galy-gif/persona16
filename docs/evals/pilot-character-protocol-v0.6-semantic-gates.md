...
1. 生成前语义控制：当前输入编译为 `TurnFrame`，关系证据投影为 active `RelationshipEffect`，统一 Policy 形成 `SemanticTurnActPlan`。
2. 单条交付门：叙事诚信、房间协议、计划动作一致性、C1 现金词汇、C8 已确认二分复刻、修复场景编造数量。违反项带具体原因进入有界重写，耗尽后不得评分。
3. 分组表达门：人物九场景、三条关系对照或房间公开消息中过度使用括号语气/动作标签时停止评分。
4. 场景语义门：C1、修复和 C8 按固定 checkId 逐项二元检查，并验证回复逐字引用。
5. 人物总体 Judge：只有前四步全部通过才运行；平均分仍用于人物完整度，不再裁决关键场景合同。
6. 关系总评与引用验证：提出 R1/R2 相对 R0 的最小事件引用。
7. 逐事件蕴含门：每个引用事件单独验证事件使用、行为变化、信息粒度和三处逐字引用。
8. 房间化学反应 Judge：只评价发言边际价值和协作；顺序依赖、参与合法性与责任落点继续由结构门判断。
9. 整批表达门：合并 36 条人物回复、12 条关系回复和房间公开消息再次检查重复标签水印。
...
- 生产全局 Prompt：`web-mvp-v8`
- 产物：`eval/artifacts/pilot-characters-v0.6.json`

具体架构理由见 [ADR-0012](../adr/0012-add-batch-and-semantic-evaluation-gates.md) 与 [ADR-0016](../adr/0016-compile-semantic-turn-control-before-generation.md)。
