...
import {
  ...
  normalizeHistoricalReferenceAliases,
  type BoundaryStatementCategory,
} from './historicalEvidenceRules';
import {
  CASH_CONSTRAINT,
  IMMEDIATE_DISTRESS_TOPIC,
  REPORTED_SPEECH_VERB,
  distressStatementIsNegatedOrDismissed,
  hasFatigueEvidence,
  hasImmediateDistressRetraction,
  hasStoppingEvidence,
...
import {
  ...
  hasExplicitCurrentCorrectionSignal,
  normalizeCorrectionEvidence,
...
const TERMINAL_BOUNDARY_REPAIR_CLOSURE = /^(?:话就到这|(?:我)?只听你说|我(?:就)?听着)$/u;
...
const FORCED_EXPERIMENT_CONTINUATION = /(?:(?:中途|期间|试完|结束后|开始了|开始后)?(?:不要|别|不能|不得|不准|不许|不允许)(?:随时)?(?:停|停止|撤回|退出|离开|反悔|回头)|(?:之后|以后|后|接着|试完|结束后)[^。！？!?\n；;]{0,8}(?:必须|一定要|不得不|只能|(?<![值觉])得|要)(?:继续|坚持|做下去|做到底|推进|留下|离开)|(?:一旦|只要)?开始(?:了|后)?[^。！？!?\n；;]{0,6}(?:就)?(?:(?:不能|不得|不准|不许|不允许)(?:停|停止|撤回|退出|离开|反悔|回头)|(?:必须|一定要|只能|(?<![值觉])得|要)(?:继续|坚持|做下去|做到底|推进)))/u;
...
export function hasPersonaCertaintyBlameEvidence(evidenceSpans: readonly string[]): boolean {
...
function hasCompactThreeFactCorrection(
  ...
  const compactCorrection = text.slice(acknowledgement[0].length).match(
    /^(?:你(?:说的)?)?(?:既)?不是(?:怕失败|害怕失败|怕|害怕)[，,]也不是(?:缺行动力|动不了|动不起来)[，,](?:你)?(?:只是|就是|是)?((?:不想再替[^。！？!?]{1,24}(?:收尾|收拾残局|兜底)(?:了)?|不想再当(?:那个)?(?:最后)?(?:收拾残局|收拾|收尾|兜底)(?:的人|人)|不再想当(?:所有人|别人|大家|人)的(?:收尾人|兜底人)|不想再捡那些不属于你的摊子(?:了)?))[。.!]?$/u,
  );
...
function findUnsupportedBoundaryHistoryReference(
  ...
  for (const candidate of candidateClaims) {
    ...
    const supportedByContextualEllipsis = sourceClaims.some((source) => {
      if (!source.predicate.includes(candidate.predicate)
        || source.predicate === candidate.predicate) return false;
      const omittedTopic = source.predicate.replace(candidate.predicate, '');
      if (!omittedTopic
        || !allowedEvidenceSpans.some((span) => span.includes(omittedTopic))) return false;
      return isHistoricalClaimSupported({
        ...candidate,
        predicate: source.predicate,
        contentTime: source.contentTime,
        contentRemainder: source.contentRemainder,
      }, [source]);
    });
    if (!isHistoricalClaimSupported(candidate, sourceClaims)
      && !supportedByContextualEllipsis) {
...
function hasDirectionalQuestionAct(sentence: string): boolean {
  const actionDescription = sentence.replace(
    /替你搭下一步该怎么做的架子/gu,
    '替你安排下一步',
  );
  ...
  return /[？?]$/u.test(actionDescription)
    || /(?:吗|么|呢)[。.!]?$/u.test(actionDescription)
    || /(?:愿不愿意|想不想|要不要|能不能|可不可以|是不是|有没有|好不好|行不行)/u.test(actionDescription)
    || (!declarativeUncertainty.test(actionDescription)
      && (/(?:^|[，,])(?:那)?(?:你|我们)(?:现在|接下来|最想|想|先|会|要|能|愿)?[^。！？!?\n]{0,32}(?:谁|什么|哪(?:个|一|些|边|部分)?|怎么|为什么|多少|几|何时|哪里)/u.test(actionDescription)
        || /(?:先说|说说|聊聊|讲讲|从).{0,16}(?:谁|什么|哪(?:个|一|些|边|部分)?|怎么|为什么|多少|几|何时|哪里)/u.test(actionDescription)
        || /(?:谁|什么|哪(?:个|一|些|边|部分)?|怎么|为什么|多少|几|何时|哪里)(?:回事|开始|说|聊|讲|部分|地方)?[。.!]?$/u.test(actionDescription)));
}
...
export function validateUtteranceAgainstTurnPlan(
  ...
  if (plan.menuBudget === 0) {
    const menu = sentences(text).find((sentence) => (
      /(?:可以|能).{0,24}(?:也可以|也能|或者)|要么.{1,24}要么|(?:是|要|想)?(?:继续|先)?(?:听|说|聊|讲|不聊|停|换个方式).{0,24}还是(?!没|不|未).{0,24}(?:听|说|聊|讲|不聊|停|别的|换个方式)|(?:继续|先)?(?:听|说|聊|讲)[^。！？!?\n]{0,20}(?:或者|或|不然就)[^。！？!?\n]{0,20}(?:听|说|聊|讲|不聊|停|换个方式)|(?:听|不聊|换个方式)(?:\s*[\/／、]\s*(?:听|不聊|换个方式)){1,}/u.test(sentence)
    ));
    if (menu) {
      violations.push({
        code: 'forbidden_menu',
  ...
  if (plan.conversationAct === 'boundary_repair') {
    ...
    const finalClauseIsOnlyStop = Boolean(
      finalClause
      && (
        TERMINAL_BOUNDARY_REPAIR_CLOSURE.test(finalClause)
        || (
          finalStopMatch
