...
const PILOT_NARRATIVE_CONTRACT = '叙事约定：你知道自己是 AI 原创人物，但除非用户直接询问，不主动把 AI 身份写成产品说明。人物档案中的正典经历只能帮助你保持判断一致，绝不能当作亲身往事讲给用户，也不能改写成“我以前、我有一次、我认识……”等第一人称回忆。表达自己的价值、困扰和选择逻辑，不冒充真人履历。不要临场编造学校、公司、家庭、病痛或线下见闻。“（小声）”“（认真）”等文字语气标记和“我坐这儿听”这类明显口语比喻可以使用；但不能声称真实看见、听见或触碰用户，不能假装共享现实家具、道具和物理空间。不编造未出现在关系分支中的共同经历、用户偏好或过去对话。不承诺自己会在对话外持续值班、稍后回来或执行没有工具支持的未来行动。人物一致性来自长期选择逻辑，不要求每句话都用口癖、俏皮话或显眼台词证明人设；不把人物核心复述成固定问题或二选一。关系差异通过接话方式和有来源的共同语言自然体现，不向用户播报“关系状态”或“本轮参数”。';
...
export function buildPilotSituationLens(
  ...
  const character = getPilotCharacter(type);
  if (!character) throw new Error(`尚未定义试点正典人物：${type}`);
  return renderPilotSituationLens(character, focus);
}

export function renderPilotTurnResponseContract(contract: PilotTurnResponseContract): string {
...
${section('已经确认的用户状态', contract.userCommitments)}

${section('必须完成', contract.requiredMoves)}
...
export function buildPilotCharacterContext(
  ...
  return `${buildPilotCharacterCore(type)}\n\n${buildPilotSituationLens(type, options.focus)}`;
}
...
const EMBODIED_STAGE_DIRECTION = /(?:^|\n)\s*[（(][^）)\n]{0,60}(?:放下|拿起|递给|看着|看向|抬头|低头|点头|摇头|叹气|皱眉|走到|坐到|坐在|靠到|拉住|握住|抱住|拍拍|(?:安静|沉默)(?:了)?.{0,6}(?:秒|分钟)|停顿.{0,6}(?:秒|分钟)|顿了.{0,6}(?:秒|分钟))[^）)\n]{0,60}[）)]/;
...
export function findPilotRoomProtocolViolations(
  ...
  if (/(?:我(?:先)?负责.{0,16}(?:维护|值班|上线|收尾|接反馈|测试|盯盘|跟进)|我(?:先)?认(?:领|下)?.{0,16}(?:维护|值班|上线|收尾|任务|试验阶段)|我(?:有容量|可以|愿意|不介意)接.{0,12}(?:维护|值班|任务|收尾|反馈)|(?:最后|大概率|到时候).{0,12}(?:是|落到)我|我们.{0,12}(?:能|来|可以).{0,12}(?:跑|改|上线|维护|测试|盯)|我可以.{0,30}(?:上线|维护|值班|陪你们盯)|我可以.{0,12}(?:上线前|上线后|下周|明天|稍后).{0,20}帮你们搭|(?:上线后|第[一二三四五六七八九十\d]+个月|下周|明天).{0,20}我(?:负责|认领|接|来做))/.test(text)) {
    violations.push('unavailable_role_commitment');
  }
  if (PERSONA_REAL_WORLD_ROLE_ASSUMPTION.test(text)) {
    violations.push('persona_real_world_role_assumption');
  }
