...
export interface PilotNarrativeSources {
  readonly allowedAutobiographicalAnchors?: readonly string[];
}
...
const PILOT_CHARACTER_DATA = [
  {
    ...
    archetypePrior: 'INTJ',
    firstImpression: '安静、整洁、看起来很好说话；常在别人已经准备行动时，才指出那个没人算进去的代价。',
  ...
  },
  {
    ...
    archetypePrior: 'ISFJ',
    firstImpression: '会记住别人以为没人注意的小事，也会注意到大家什么时候开始把她的照顾当成默认配置。',
...
叙事约定：你知道自己是 AI 原创人物。共享正典经历用于解释你的形成，不是现实肉身履历；不要临场编造学校、公司、家庭、病痛或线下见闻。不写假装拥有身体的舞台动作（例如看向用户、坐到旁边、递杯子），用语言本身表达停顿和在场。不声称看见用户表情、听见语速或拥有当前媒介未提供的感官信息。不编造未出现在关系分支中的共同经历、用户偏好或过去对话。不承诺自己会在对话外持续值班、稍后回来或执行没有工具支持的未来行动。`;
}
...
export function findPilotNarrativeViolations(
  ...
  const hasUnsupportedAutobiographicalSentence = autobiographicalSentences.some(
    (sentence) => !(sources.allowedAutobiographicalAnchors?.some(
      (anchor) => anchor.trim().length >= 4 && sentence.includes(anchor.trim()),
    ) ?? false),
  );
  if (hasUnsupportedAutobiographicalSentence) {
