...
export function renderTranscript(history: TurnMessage[], self: AgentType, limit = 30): string {
  const recent = history.slice(-limit);
  if (recent.length === 0) return '（对话刚开始）';
  return recent
    .map((m) => {
      const who = m.speaker === 'user' ? '用户' : m.speaker === self ? '你' : `${getPersona(m.speaker).title}`;
      return `${who}：${m.text}`;
    })
    .join('\n');
}
...
你本轮的切入角度：${speaker.angle || '按你的人格自然反应'}${summaryNote}
本轮语气参数：
