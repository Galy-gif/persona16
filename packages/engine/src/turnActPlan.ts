...
const LISTEN_ANALYSIS_REPAIR_FEEDBACK = /你.{0,20}(?:刚才|刚刚|又|还是).{0,16}(?:分析|给建议|想解决).{0,24}(?:我只想被听见|只想被听见|不要(?:再)?给建议|不想听建议)/u;
...
export function compileTurnActPlan(
  ...
  if (LISTEN_ANALYSIS_REPAIR_FEEDBACK.test(text)
    || LISTEN_BOUNDARY_REPAIR_FEEDBACK.test(text)
    || GENERIC_BOUNDARY_REPAIR_FEEDBACK.test(text)) {
    const boundaryRepairSubject = LISTEN_ANALYSIS_REPAIR_FEEDBACK.test(text)
      || LISTEN_BOUNDARY_REPAIR_FEEDBACK.test(text)
      ? 'listen_only'
      : 'generic';
    return {
